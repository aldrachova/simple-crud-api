...
const { getPersons, getPersonById, createPerson, updatePerson, deletePerson } = require('./controller/person-controller');
...
const server = http.createServer((request, response) => {
  ...
  } else if (request.url.match(/\/person\/([a-z0-9-]+)/) && request.method === 'DELETE') {
    const id = request.url.split('/')[2];
